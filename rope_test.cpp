...
public:
    ...
    Rope(std::string s,int key, Rope* left, Rope* right, Rope* parent)
            : s(s), res(s), key(key), left(left), right(right), parent(parent) {}
...
void split(Rope* root, int key, Rope*& left, Rope*& right) {
    ...
    update(left);
    update(right);
    update_key(right, left != NULL ? -(left->res.length()) : 0);
}
...
void insert(int x, char s) {
    ...
    if (right == NULL || right->key != x) {
        new_rope = new Rope(t, x, NULL, NULL, NULL);
    }
...
void process( int i, int j, int k ) {
    ...
    update_key(right, middle != NULL ? middle->res.length() : 0);
    middle = merge(middle, right);
    update_key(middle, left != NULL ? left->res.length() : 0);
